...
interface MemberTableRowProps {
  ...
  isExpanded: boolean;
  hasActivityFlag: boolean;
  TABLE_COLSPAN: number;
  toggleExpanded: (id: number) => void;
  copyBounty: (name: string, id: number) => void;
}

// Wrapped in React.memo to prevent O(N) re-renders when parent state (like expandedId) changes
        ...
        <tr className="bg-bg-elevated/30">
          <td colSpan={TABLE_COLSPAN} className="py-3 px-4">
            <div className="space-y-3 max-w-3xl">
              {hasActivityFlag && <MemberActivityPanel playerId={m.id} />}
            </div>
          </td>
        </tr>
      )}
...
export function MemberTable({ members, detail, overview }: MemberTableProps) {
  ...
  // Memoized to prevent invalidating React.memo on child rows
          ...
          <tbody>
            {sorted.map((m) => (
              <MemberTableRow
                ...
                hasActivityFlag={flags.activity}
