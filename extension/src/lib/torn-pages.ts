...
export type PageKind = 'profile' | 'attack' | 'bounties' | 'stocks' | 'faction' | 'hospital' | 'armoury' | 'retals' | 'travel' | 'ambient' | 'imarket' | 'oc' | 'jail' | 'halloffame' | 'poker' | 'unknown';
...
export function matchPage(url: URL = new URL(window.location.href)): PageMatch {
  ...
  if (path === '/loader.php' || path === '/page.php') {
    ...
    if (sid === 'jailView' || sid === 'jail') {
      return { kind: 'jail', player_id: null };
    }
    if (sid === 'poker' || sid === 'holdem') {
      return { kind: 'poker', player_id: null };
    }
  }
  if (path === '/casino.php' && url.searchParams.get('page') === 'poker') {
    return { kind: 'poker', player_id: null };
  }
  ...
  if (path === '/jailview.php') {
    return { kind: 'jail', player_id: null };
  }
  if (path === '/halloffame.php') {
    return { kind: 'halloffame', player_id: null };
  }
  ...
  if (path === '/factions.php') {
    ...
    if (step === 'crimes' || step === 'oc') {
      return { kind: 'oc', player_id: null };
    }
  }
...
export const OC_ANCHOR_SELECTORS = [
  '#faction-crimes',
  '#faction-crimes-root',
  '#mainContainer .content-wrapper',
  '#mainContainer',
